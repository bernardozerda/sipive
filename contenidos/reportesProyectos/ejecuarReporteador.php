...
if (empty($arrSCampos) and $bolReporteCuenta === false) {
    ...
    foreach ($arrSCampos as $txtAliasCampoAnalizar => $txtCampoAnalizar) {
        ...
        if ($arrDatosCampo["tablaOrigen"] == "T_PRY_ENTIDAD_OFERENTE") {
            $txtInnerJoin = "";
            $txtInnerJoin = "LEFT JOIN t_pry_proyecto_oferente proyOf ON pry.seqProyecto = proyOf.seqProyecto"
                    . " LEFT JOIN t_pry_entidad_oferente ofe  ON proyOf.seqOferente = ofe.seqOferente";
            ...
            if ($txtCampoAnalizar == "seqUsuario") {
                ...
                switch ($txtTipoDato) {
                    case "externo":
                        ...
                        break;
                    case "texto":
                        $arrCamposSql[] = "group_concat(upper(" . $arrDatosCampo["aliasOrigen"] . "." . $txtCampoAnalizar . "), '<br>') " .
                                "as '" . $arrDatosCampo["nombre"] . "'";
                        break;
                    case "numero":
