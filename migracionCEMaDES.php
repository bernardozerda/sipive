<?php
session_start();
if ( $_SESSION['seqUsuario'] == 3 || $_SESSION['seqUsuario'] == 5 || $_SESSION['seqUsuario'] == 218 || $_SESSION['seqUsuario'] == 414 ) { ?>
    <!DOCTYPE html>
                    ...
                    <div class="caption">
                        <form enctype="multipart/form-data" action="<?php $_SERVER['PHP_SELF'] ?>" method="POST">
                            <table cellpadding="2" cellspacing="0" border="0" width="600">
                                <tr>
                                    <td colspan="2" bgcolor="#E4E4E4" class="tituloTabla" align="center"><h3>Migraci&oacute;n
                                            CEM a Desembolso</h3></td>
                                </tr>
                                <tr>
                                    <td><b>Seleccione el archivo:</b><br>En el archivo plano debe ir la lista de los
                                        documentos sin encabezado
                                    </td>
                                    <td valign="top"><input type="file" name="fileDocumentos" class="file"/></td>
                                </tr>
                                <tr>
...
//                                        echo "<b>Estado Proceso: </b>[" . $rowForm['seqEstadoProceso'] . "]<br>";
                                        ...
                                        if ($idEstadoProceso == 15 || $idEstadoProceso == 62) { // inicio if 2
                                            // MIEMBROS DEL HOGAR
                                            $sqlMiembros = $aptBd->execute("SELECT T_FRM_HOGAR.seqCiudadano, seqParentesco, numDocumento, seqTipoDocumento, CONCAT(txtNombre1,' ',txtNombre2,' ',txtApellido1,' ',txtApellido2) AS nombre FROM T_FRM_HOGAR LEFT JOIN T_CIU_CIUDADANO ON (T_FRM_HOGAR.seqCiudadano = T_CIU_CIUDADANO.seqCiudadano) WHERE seqFormulario = '" . $rowForm['seqFormulario'] . "' AND seqParentesco = 1");
                                            while ($rowMiembros = $sqlMiembros->fields) {
                                                ...
                                                $sqlMiembros->MoveNext();
                                            }
                                            // DATOS DEL DESEMBOLSO
                                            $sqlDesemb = $aptBd->execute("SELECT seqDesembolso FROM T_DES_DESEMBOLSO WHERE seqFormulario = '" . $idFormulario . "'");
                                            $rowDesemb = $sqlDesemb->fields;
                                            $codigoDesembolso = $rowDesemb['seqDesembolso'];
                                            //echo "<b>Desembolso: </b>[" . $codigoDesembolso . "]<br>";
                                            $codigoDesembolso = $rowDesemb['seqDesembolso'];
                                            // DATOS CASA EN MANO
                                            $sqlMaxCasaMano = $aptBd->execute("SELECT MAX(seqCasaMano) as maximoCEM FROM T_CEM_CASA_MANO WHERE seqFormulario = '" . $idFormulario . "'");
                                            $rowMaxCasaMano = $sqlMaxCasaMano->fields;
								...
								WHERE  seqCasaMano = $codigoCasamano";
                                                    $aptBd->execute($sqlDesembolso);
                                                    // Identifica el codigo de desembolso creado
                                                    $sqlUltimoDesembolso = $aptBd->execute("SELECT MAX(seqDesembolso) AS maximo FROM T_DES_DESEMBOLSO WHERE seqFormulario = $idFormulario");
                                                    $rowUltimoDesembolso = $sqlUltimoDesembolso->fields;
								...
								WHERE  seqCasaMano = $codigoCasamano";
                                                ...
                                                $aptBd->execute($sqlTecnico);

                                                // Identifica el codigo seqTecnico creado
                                                $sqlUltimoTecnico = $aptBd->execute("SELECT MAX(seqTecnico) AS maximoTecnico FROM T_DES_TECNICO WHERE seqDesembolso = $codigoDesembolso");
                                                $rowUltimoTecnico = $sqlUltimoTecnico->fields;
								...
								FROM   T_CEM_JURIDICO
								WHERE  seqCasaMano = $codigoCasamano";
                                                $aptBd->execute($sqlJuridico);

                                                // Identifica el codigo seqJuridico creado
                                                $sqlUltimoJuridico = $aptBd->execute("SELECT MAX(seqJuridico) AS maximoJuridico FROM T_DES_JURIDICO WHERE seqDesembolso = $codigoDesembolso");
                                                $rowUltimoJuridico = $sqlUltimoJuridico->fields;
                                                ...
                                                $sqlSeqTecnicoCEM = $aptBd->execute("SELECT seqTecnico FROM T_CEM_TECNICO WHERE seqCasaMano = $codigoCasamano");
                                                $rowSeqTecnicoCEM = $sqlSeqTecnicoCEM->fields;
                                                $codigoTecnicoCEM = $rowSeqTecnicoCEM['seqTecnico'];

                                                // Obtiene codigo de seqJuridico en tabla CEM
                                                $sqlSeqJuridicoCEM = $aptBd->execute("SELECT seqJuridico FROM T_CEM_JURIDICO WHERE seqCasaMano = $codigoCasamano");
                                                $rowSeqJuridicoCEM = $sqlSeqJuridicoCEM->fields;
                                                $codigoJuridicoCEM = $rowSeqJuridicoCEM['seqJuridico'];

                                                // Adjuntos Tecnicos de CEM
                                                $sqlAdjuntosTecnicosCEM = $aptBd->execute("SELECT seqAdjuntoTecnico FROM T_CEM_ADJUNTOS_TECNICOS WHERE seqTecnico = $codigoTecnicoCEM");
                                                while ($rowAdjuntosTecnicos = $sqlAdjuntosTecnicosCEM->fields) {
												...
												SELECT '$codigoTecnico', $camposAdjuntosTecnicos
                                                    ...
                                                    $aptBd->execute($sqlAdjuntosTecnicos);
                                                    $sqlAdjuntosTecnicosCEM->MoveNext();
                                                }

                                                // Adjuntos Juridicos de CEM
                                                $sqlAdjuntosJuridicosCEM = $aptBd->execute("SELECT seqAdjuntoJuridico FROM T_CEM_ADJUNTOS_JURIDICOS WHERE seqJuridico = $codigoJuridicoCEM");
                                                while ($rowAdjuntosJuridicos = $sqlAdjuntosJuridicosCEM->fields) {
												...
												SELECT '$codigoJuridico', $camposAdjuntosJuridicos
                                                    ...
                                                    $aptBd->execute($sqlAdjuntosJuridicos);
                                                    $sqlAdjuntosJuridicosCEM->MoveNext();
                                                }
                ...
                <footer>
                    <div class="well well-small">
                        <center>
                            ...
                            <h6>Para visualizar mejor este sitio se recomienda el uso de Chrome, Mozilla Firefox ó
                                Internet Explorer 10.</h6>
                        </center>
                    </div>
                </footer>
            </div>
    ...
    <script language="JavaScript" type="text/javascript"
            src="./librerias/bootstrap/js/bootstrap-transition.js"></script>
