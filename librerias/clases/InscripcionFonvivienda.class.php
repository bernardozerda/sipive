...
//                        pr($claFormularioOriginal);
//                        pr($claFormulario);
//                        pr($claFormularioSeparado);
    ...
    public function procesarCargue_old2($seqCargue){
        global $aptBd;
        ...
        try{
            ...
            foreach($this->arrHogares as $numHogar => $arrHogar) {
                ...
                $claFormulario = null;
                $claFormularioSeparado = null;

                $numDocumentoSeguimiento = 0;
                $txtNombreSeguimiento = "";

                $numDocumentoSeparado = 0;
                $txtNombreSeparado = "";
                ...
                if($arrHogar['seqFormulario'] == 0){
                    ...
                    // valores del formulario
                    unset($arrHogar['seqFormulario']);
                    foreach($claFormulario as $txtCampo => $txtValor){
                        if(isset($arrHogar[$txtCampo])){
                            $claFormulario->$txtCampo = $arrHogar[$txtCampo];
                        }
                    }
                    $claFormulario->guardarFormulario();
                    ...
                    $claformularioOriginal = $claFormulario;
                    ...
                    // fusiona MCY --> separa VIPA y EPI
                    if($this->seqTipo == 1){

                        $claFormulario = new FormularioSubsidios();
                        $claFormulario->cargarFormulario($arrHogar['seqFormulario']);

                        // copia la informacion del ciudadano existente
                        foreach($arrHogar['ciudadanos'] as $idCiudadano => $arrCiudadano){
                            if($arrCiudadano['seqParentesco'] == 1){
                                $numDocumentoSeguimiento = $arrCiudadano['numDocumento'];
                                $txtNombreSeguimiento    = $arrCiudadano['txtNombre1'] . " " . $arrCiudadano['txtNombre2'] . " " . $arrCiudadano['txtApellido1'] . " " . $arrCiudadano['txtApellido2'];
                            ...
                            foreach($claFormulario->arrCiudadano as $seqCiudadano => $objCiudadano){
                                if(
                                    $arrCiudadano['seqTipoDocumento'] == $objCiudadano->seqTipoDocumento and
                                    $arrCiudadano['numDocumento'] == $objCiudadano->numDocumento
                                ){
                                    $objCiudadano = $this->modificarCiudadano($objCiudadano,$arrCiudadano);
                                    $objCiudadano->editarCiudadano($objCiudadano->seqCiudadano);
                                    $claFormulario->arrCiudadano[$seqCiudadano] = $objCiudadano;
                        ...
                        if(! empty($arrHogar['ciudadanos'])){
                            foreach($arrHogar['ciudadanos'] as $idCiudadano => $arrCiudadano){
                                ...
                                $seqCiudadano = $claCiudadano->seqCiudadano;
                                $claFormulario->arrCiudadano[$seqCiudadano] = $claCiudadano;
                                if(doubleval($numDocumentoSeparado) == 0){
                                    $numDocumentoSeparado = $arrCiudadano['numDocumento'];
                                    $txtNombreSeparado    = $arrCiudadano['txtNombre1'] . " " . $arrCiudadano['txtNombre2'] . " " . $arrCiudadano['txtApellido1'] . " " . $arrCiudadano['txtApellido2'];
                                }
                            }
                        }
                        ...
                        // datos del formulario
                        $claFormulario = $this->modificarFormulario($claFormulario, $arrHogar);
                        $claFormulario->editarFormulario();
                        ...
                        $claFormularioSeparado = new FormularioSubsidios();
                        $claFormularioSeparado->cargarFormulario($arrHogar['seqFormulario']);
                        $claFormularioSeparado->arrCiudadano = array();

                        foreach($claFormulario->arrCiudadano as $seqCiudadano => $objCiudadano){
                            $bolPresente = false;
                            foreach($arrHogar['ciudadanos'] as $idCiudadano => $arrCiudadano){

                                echo $objCiudadano->seqTipoDocumento ." == " . $arrCiudadano['seqTipoDocumento']  ." and " .  $objCiudadano->numDocumento  ." == " .  $arrCiudadano['numDocumento'] . " => ";
...
//                                pr($objCiudadano);
                        ...
                        // datos del formulario separado
                        $claFormularioSeparado = $this->modificarFormulario($claFormulario, $arrHogar);

                        // reset de datos del formulario separado
                        $claFormularioSeparado->seqBancoCuentaAhorro = 1;
                        $claFormularioSeparado->fchAperturaCuentaAhorro	= null;
                        $claFormularioSeparado->bolInmovilizadoCuentaAhorro	= 0;
                        $claFormularioSeparado->valSaldoCuentaAhorro = 0;
                        $claFormularioSeparado->txtSoporteCuentaAhorro = '';
                        $claFormularioSeparado->seqBancoCuentaAhorro2 = 1;
                        $claFormularioSeparado->fchAperturaCuentaAhorro2 = null;
                        $claFormularioSeparado->bolInmovilizadoCuentaAhorro2 = 0;
                        $claFormularioSeparado->valSaldoCuentaAhorro2 = 0;
                        $claFormularioSeparado->txtSoporteCuentaAhorro2 = '';
                        $claFormularioSeparado->valSubsidioNacional = 0;
                        $claFormularioSeparado->seqEntidadSubsidio = 1;
                        $claFormularioSeparado->txtSoporteSubsidio = '';
                        $claFormularioSeparado->valAporteLote = 0;
                        $claFormularioSeparado->txtSoporteAporteLote = '';
                        $claFormularioSeparado->seqCesantias = 1;
                        $claFormularioSeparado->valSaldoCesantias = 0;
                        $claFormularioSeparado->txtSoporteCesantias = '';
                        $claFormularioSeparado->valAporteAvanceObra = 0;
                        $claFormularioSeparado->txtSoporteAvanceObra = '';
                        $claFormularioSeparado->valAporteMateriales = 0;
                        $claFormularioSeparado->txtSoporteAporteMateriales = '';
                        $claFormularioSeparado->valDonacion = 0;
                        $claFormularioSeparado->txtSoporteDonacion = '';
                        $claFormularioSeparado->seqBancoCredito = 1;
                        $claFormularioSeparado->valCredito = 0;
                        $claFormularioSeparado->txtSoporteCredito = '';
                        $claFormularioSeparado->valTotalRecursos = 0;
                        $claFormularioSeparado->valAspiraSubsidio = 0;
                        $claFormularioSeparado->fchPostulacion = null;
                        $claFormularioSeparado->fchVencimiento = null;
                        $claFormularioSeparado->bolCerrado = 0;
                        $claFormularioSeparado->valIngresoHogar = 0;
                        $claFormularioSeparado->seqEstadoProceso = 35;
                        $claFormularioSeparado->txtDireccionSolucion = '';
                        $claFormularioSeparado->seqEmpresaDonante = 1;
                        $claFormularioSeparado->fchAprobacionCredito = null;
                        $claFormularioSeparado->txtFormulario = '';
                        $claFormularioSeparado->numCortes = 0;
                        $claFormularioSeparado->seqProyecto = 0;
                        $claFormularioSeparado->seqProyectoHijo = 0;
                        $claFormularioSeparado->seqTipoFinanciacion = 1;
                        $claFormularioSeparado->seqPeriodo = null;
                        $claFormularioSeparado->txtSoporteSubsidioNacional = '';
                        $claFormularioSeparado->bolSancion = 0;
                        $claFormularioSeparado->fchVigencia = null;

                        $claFormularioSeparado->editarFormulario();
                ...
                $claSeguimiento = new Seguimiento();
                $txtCambios = $claSeguimiento->cambiosPostulacionActo($claFormulario->seqFormulario,$claFormularioOriginal,$claFormulario);
                $txtCambiosSeparacion = "";
                if($claFormularioSeparado !== null) {
                    $txtCambiosSeparacion = $claSeguimiento->cambiosPostulacionActo($claFormulario->seqFormulario, $claFormularioOriginal, $claFormularioSeparado);
                }

//                echo "Cambios => ";
//                pr($txtCambios);
//                echo "CambiosSeparacion => ";
//                pr($txtCambiosSeparacion);
                ...
                if($claFormularioSeparado !== null){
                    $sql = "
                     INSERT INTO T_SEG_SEGUIMIENTO (
                       ...
                       " . $claFormularioSeparado->seqFormulario . ",
                       '" . date("Y-m-d H:i:s") . "',
                       " . $_SESSION['seqUsuario'] . ",
                       '" . $txtComentario . "',
                       '" . mb_ereg_replace("'","\'",$txtCambiosSeparacion) . "',
                       " . $numDocumentoSeparado . ",
                       '" . $txtNombreSeparado . "',
                       " . 46 . "
                     )
                 ";
    ...
    public function procesarCargue_old($seqCargue){
        global $aptBd;
