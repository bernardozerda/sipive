...
if( $_FILES['fileUnidadesActo']['error'] != UPLOAD_ERR_NO_FILE ){
	...
	if( empty( $arrErrores ) ){
		...
		$ultimoActo = $aptBd->Insert_ID();
                $sqlAddUndVinculadas = "INSERT T_PRY_AAD_UNIDADES_VINCULADAS (seqUnidadActo, seqUnidadProyecto, valIndexado) VALUES";
                $int =0;
                $sqlUpdateSubsidio = "UPDATE T_FRM_FORMULARIO SET valAspiraSubsidio = CASE seqUnidadProyecto";
                $valSDVEActual = "";
                $txtSoporteSubsidio =" END, txtSoporteSubsidio = CASE seqUnidadProyecto";
                $idSeqUnidadProyecto ="";
		...
		foreach ($arrDatosCombinados AS $seqUnidadProyecto => $arrUnidadAuxiliar){
			...
			if($int < count($arrDatosCombinados)){
                            $idSeqUnidadProyecto .= $arrUnidadAuxiliar['seqUnidadProyecto'].",";
                            $sqlAddUndVinculadas .= " ($ultimoActo, " . $arrUnidadAuxiliar['seqUnidadProyecto'] . ", " . $arrUnidadAuxiliar['valIndexacion'] . "), ";
                        }else{
                            $idSeqUnidadProyecto .= $arrUnidadAuxiliar['seqUnidadProyecto']."";
                            $sqlAddUndVinculadas .= " ($ultimoActo, " . $arrUnidadAuxiliar['seqUnidadProyecto'] . ", " . $arrUnidadAuxiliar['valIndexacion'] . ") ";
			...
			if ($arrUnidadAuxiliar['seqFormulario'] > 0) { // Si existe el formulario actualice los datos del subsidio
				$valSDVEActual .= " WHEN ".$arrUnidadAuxiliar['seqUnidadProyecto'] ." THEN ".$nuevoValorActual;
                                $txtSoporteSubsidio .= " WHEN ".$arrUnidadAuxiliar['seqUnidadProyecto'] ." THEN '".$soporte."'";
                            
                         /* echo "<br>". $sqlUpdateSubsidio1 = "UPDATE T_FRM_FORMULARIO 
                                                                            SET valAspiraSubsidio = " . $nuevoValorActual . ", txtSoporteSubsidio = '" . $soporte . "'
                                                                            WHERE seqUnidadProyecto = " . $arrUnidadAuxiliar['seqUnidadProyecto'] . "";*/
				///$aptBd->execute($sqlUpdateSubsidio);
			}
		} // Fin foreach
                $sqlForm = $sqlUpdateSubsidio ."".$valSDVEActual."".$txtSoporteSubsidio ." END WHERE seqUnidadProyecto IN (".$idSeqUnidadProyecto.")";
                //echo "<br>".$sqlForm;
               $aptBd->execute($sqlAddUndVinculadas);
               $aptBd->execute($sqlForm);
	} // Fin If errores
}
	///////////////////////////////////////////////////////////////////////// IMPRIMIR MENSAJES
	if( empty( $arrErrores ) ){
