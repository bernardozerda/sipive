...
class Usuario {
    ...
    public $seqDependencia;
    public $txtMatriculaProfesional;
    ...
    function Usuario() {
        ...
        $this->bolBorrar = 0;
        $this->bolCambiar = 0;
        $this->seqDependencia = 0;
        $this->txtMatriculaProfesional = "";
    }
    ...
    public function cargarUsuario($seqUsuario = 0) {
        ...
        try {
            ...
            while ($objRes->fields) {
                ...
                $objUsuario->seqDependencia = $objRes->fields['seqDependencia'];
                $objUsuario->txtMatriculaProfesional = $objRes->fields['txtMatriculaProfesional'];
...
// Fin Cargar usuario
     ...
     */
    public function guardarUsuario($txtNombre, $txtApellido, $txtUsuario, $txtClave, $txtCorreo, $numEstado, $numVencimiento, $arrGrupos, $seqDependencia, $txtMatriculaProfesional, $arrPrivilegios) {
        ...
        $sql = "
                INSERT INTO T_COR_USUARIO (
                    ...
                    " . $seqDependencia . ",
                    \"" . ereg_replace('\"', '', $txtMatriculaProfesional) . "\"                       
                )
            ";
...
// fin guardar usuario
     ...
     * @version 1.0 Abril de 2009
     */
    public function editarUsuario($seqUsuario, $txtNombre, $txtApellido, $txtUsuario, $txtClave, $txtCorreo, $numEstado, $numVencimiento, $arrGrupo, $seqDependencia, $txtMatriculaProfesional, $arrPrivilegios) {
        ...
        if (empty($arrErrores)) {
            $sql = "
                    UPDATE T_COR_USUARIO SET 
                        ...
                        seqDependencia = " . $seqDependencia . ",
                        txtMatriculaProfesional = \"" . mb_ereg_replace('\"', '', $txtMatriculaProfesional) . "\"    
                    WHERE seqUsuario = $seqUsuario
                ";
            try {
                $aptBd->execute($sql);
...
// fin Cambiar clave
    ...
    public function obtenerDependencia() {
        ...
        $sql = "SELECT seqDependencia, concat(txtCodigo, ' - ', txtDependencia) as dependencia FROM t_cor_dependencia";
        $objRes = $aptBd->execute($sql);
        $arrDependencia = array();
        while ($objRes->fields) {
            $arrDependencia[$objRes->fields['seqDependencia']] = $objRes->fields['dependencia'];
            $objRes->MoveNext();
        }
        //var_dump($arrDependencia); die();
        return $arrDependencia;
    }
