...
function variables($valor) {
    if ($valor == 1) {
        ...
        $arrEstado[28] = "Consolidación Documental";
        $arrEstado[29] = "Cierre Legalizado";

        return $arrEstado;
    } else if ($valor == 2) {
        $listEstados = "15,62,17,19,22,23,25,26,27,28,29,40";
        return $listEstados;
    } else {
        $arrEstadoCon = array();
        //$arrEstadoCon[15] = "";
        $arrEstadoCon[62] = "SELECT count(*) as cant, 
                            ...
                            AS color
                               ...
                               where und.seqProyecto = **  and seqEstadoProceso = ¬¬
                               GROUP BY color";

        $arrEstadoCon[17] = "SELECT count(*) as cant, 
                            case WHEN (datediff(DATE(NOW()), fchInformacionSolucion))between 0 and 2
                            THEN 'verde'
                            WHEN (datediff(DATE(NOW()), fchInformacionSolucion)) between 3 and 4
                            THEN 'amarillo'
                            ELSE 'rojo' 
                            END
                            AS color
        ...
        $arrEstadoCon[19] = "SELECT count(*) as cant, 
                                case WHEN (datediff(DATE(NOW()), fchCreacionBusquedaOferta))between 0 and 2
                                THEN 'verde'
                                WHEN (datediff(DATE(NOW()), fchCreacionBusquedaOferta)) between 3 and 4
                                THEN 'amarillo'
                                ELSE 'rojo' 
                                END
                                AS color
                             ...
                             where  und.seqProyecto = **  and seqEstadoProceso = ¬¬
                             GROUP BY color";
        $arrEstado[22] = "SELECT count(*) as cant, 
        ...
        $arrEstado[23] = "SELECT count(*) as cant, 
                                case WHEN (datediff(DATE(NOW()), fchCreacionEscrituracion)) between 0 and 2
                                THEN 'verde'
                                WHEN (datediff(DATE(NOW()), fchCreacionEscrituracion)) between 3 and 3
                                THEN 'amarillo'
                                ...
                                AS color
                                 from t_pry_unidad_proyecto und
                                 INNER JOIN t_frm_formulario frm USING (seqFormulario)
                                 INNER JOIN t_des_escrituracion des USING (seqFormulario)
                             where  und.seqProyecto = **  and seqEstadoProceso = ¬¬
                             GROUP BY color";

        $arrEstado[25] = "SELECT count(*) as cant, 
                        case WHEN (datediff(DATE(NOW()), tec.fchCreacion))between 0 and 2
                        THEN 'verde'
                        WHEN (datediff(DATE(NOW()), tec.fchCreacion)) between 3 and 4
                        THEN 'amarillo'
                        ELSE 'rojo' 
                        END
                        AS color
        ...
        $arrEstadoCon[27] = "SELECT count(*) as cant, 
                            ...
                            AS color
                               ...
                               where und.seqProyecto = **  and seqEstadoProceso = ¬¬
                               GROUP BY color";
        $arrEstadoCon[28] = "SELECT count(*) as cant, 
