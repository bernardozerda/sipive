...
/**
 * POSTULACION PARA PLAN DE GOBIERNO 2 Y 3
...
if( empty( $arrErrores ) ){
    ...
    if( ! empty( $_POST['hogar'] ) ){
        ...
        foreach( $_POST['hogar'] as $numDocumento => $arrCiudadano ){
            ...
            if( ! isset($arrEstadoCivilProhibido[$seqEstadoCivil] ) ){
            ...
            if( ! esFechaValida( $arrCiudadano['fchNacimiento'] ) ){
                $arrErrores[] = "El ciudadano con numero de documento " . number_format($numDocumento) . " debe tener una fecha de nacimiento válida";
            } else {
    ...
    foreach($arrCamposCalificacion as $txtCampo){
        if($_POST[$txtCampo] != $claFormulario->$txtCampo){
