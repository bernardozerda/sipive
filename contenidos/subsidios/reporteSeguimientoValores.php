	...
	$sql = "
		SELECT
			...
			(
			  select 
			    par.txtParentesco
			  from T_CIU_PARENTESCO par
			  where par.seqParentesco = hog.seqParentesco
			  ) as Parentesco,
			moa.txtModalidad , 
		...
		WHERE (seg.txtCambios like '%valSaldoCuentaAhorro%' 
				or seg.txtCambios like '%valCredito%' 
				or seg.txtCambios like '%valSaldoCuentaAhorro2%')
                AND $txtCondicion
	";
	//echo $sql;die();
	$objRes = $aptBd->execute( $sql );
